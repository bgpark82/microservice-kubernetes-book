...
class ProductCompositeServiceImplTest {
    ...
    @MockBean
    private ProductCompositeServiceIntegration integration;

    @BeforeEach
    void setUp() {
        when(integration.getProduct(PRODUCT_ID_OK))
                .thenReturn(new Product(PRODUCT_ID_OK, "name", 1, "address"));
        when(integration.getRecommendations(PRODUCT_ID_OK))
                .thenReturn(singletonList(new Recommendation(PRODUCT_ID_OK, 1, "author", 1, "content", "address")));
        when(integration.getReviews(PRODUCT_ID_OK))
                .thenReturn(singletonList(new Review(PRODUCT_ID_OK, 1, "author","subject","content","address")));
    ...
    @DisplayName("없는 productId로 getProduct 호출 시 Not Found")
    @Test
    ...
    @DisplayName("invalid productId로 getProduct 호출 시 Invalid")
    @Test
